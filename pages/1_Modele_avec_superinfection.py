...
with col2: 
    ...
    supinfec = st.slider("Capacité de surinfection",min_value = 0.0, max_value = 100.0,step = 0.01)
...
# def model(Y0, t ,sig,supinfec,pay,c,k,A,N) :
#     #I , gamma, x = Y0
#     I =Y0[0]
#     gamma =Y0[1]
#     x =Y0[2]
...
def cooperators(i , gamma,x ,parms = [sig,supinfec,pay,c,k,A,N] ):
...
def runge_kunta_4(pas, I , gamma,x,parms = [sig,supinfec,pay,c,k,A,N],name = None):
    sig,supinfec,pay,c,k,A,N = parms
    
    if name == "gamma":
        k1 = clairance(I , gamma,x,parms = [sig,supinfec,pay,c,k,A,N])
        k2 = clairance(I ,gamma + k1*pas/2 ,x, parms = [sig,supinfec,pay,c,k,A,N])
        k3 = clairance(I ,gamma + k2*pas/2 ,x  ,parms = [sig,supinfec,pay,c,k,A,N])
        k4 = clairance(I ,gamma + k3*pas ,x  ,parms = [sig,supinfec,pay,c,k,A,N] )
        return(gamma + pas*(k1 + 2*k2 + 2*k3 +k4)/6) 
        
    if name == "coop":
        k1 = cooperators(I , gamma,x,parms = [sig,supinfec,pay,c,k,A,N])
        k2 = cooperators(I ,gamma,x + k1*pas/2 , parms = [sig,supinfec,pay,c,k,A,N] )
        k3 = cooperators(I ,gamma,x + k2*pas/2  ,parms = [sig,supinfec,pay,c,k,A,N])
        k4 = cooperators(I ,gamma,x + k3*pas   ,parms = [sig,supinfec,pay,c,k,A,N] )
...
def better_ode( tmax, pas ,Y0,parms):
    ...
    for y in range(len(t)):
        ...
        dgamma = runge_kunta_4(pas, i , gamma,x,parms = [sig,supinfec,pay,c,k,A,N], name = "gamma" )
        dx = runge_kunta_4(pas, i , gamma,x,parms = [sig,supinfec,pay,c,k,A,N], name = "coop" )
...
# sol = solve_ivp(model, y0 = [i0 , c0,x0], t_span = (0,tmax),args = (sig,supinfec,pay,c,k,A,N),method="RK45",dense_output=True)
# sol = sol.y

# sol = odeint(model, y0 = [i0 , c0,x0], t=temps,args = (sig,supinfec,pay,c,k,A,N))

sol = better_ode( tmax , pas ,Y0 = [i0 , c0,x0],parms =[sig,supinfec,pay,c,k,A,N])
# (Y0, tmax, pas ,parms)
